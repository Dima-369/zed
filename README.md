...
- restore last file finder query when reopening the file finder modal, so the previous search text is pre-filled and fully selected (blue highlight), meaning typing instantly replaces it while arrow keys break out of the selection (JetBrains IDEs' behavior)
...
- add `editor_tab_label` task template field: optional label for the terminal tab title. Same variable substitution as `label`. When set, the tab title uses this instead of `label`, while `label` stays untouched for the command palette. Lets you write `"editor_tab_label": "Run $ZED_DIRNAME_BASE/$ZED_FILENAME"` to distinguish parallel task runs. Implemented in `TaskTemplate` + `resolve_task()` (`crates/task/src/task_template.rs`); falls back to `label` when absent/empty.
- add `$ZED_DIRNAME_BASE` task variable: last segment of the parent directory. For `/home/project/src/index.php`, this is `src`. Useful in tab labels where the full `$ZED_DIRNAME` is too verbose. Populated in `task_inventory.rs` alongside `$ZED_DIRNAME`.
