...
async fn test_expand_first_line_diff_hunk_keeps_deleted_lines_visible(
    ...
    cx.update_editor(|editor, window, cx| {
        editor.scroll(gpui::Point { x: 0., y: 0. }, None, false, window, cx);
    });
    executor.run_until_parked();
...
#[gpui::test]
async fn test_undo_restores_cursor_position_after_paste_at_line_end(cx: &mut TestAppContext) {
    init_test(cx, |_| {});
    ...
    cx.update_editor(|editor, window, cx| {
        let buffer_snapshot = editor.buffer.read(cx);
    ...
    cx.assert_editor_state("wordpastedˇ");
    ...
    cx.assert_editor_state("wordˇ"); // This should pass, but currently fails due to the bug
}
...
async fn test_sticky_scroll(cx: &mut TestAppContext) {
    ...
    let mut sticky_headers = |offset: ScrollOffset| {
        cx.update_editor(|e, window, cx| {
            e.scroll(gpui::Point { x: 0., y: offset }, None, false, window, cx);
